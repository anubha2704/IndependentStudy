...
import pandas as pd
############################# data processing #####################################################
#change 0 and 1 for spam, non-spam
...
#feature2_cols.extend(other_cols)
print("read data")
#features of Labelled data - Train 
data = pd.read_csv('labeledFeat.tsv', sep='\t')#nrows = 100)
data['Label'] = data['Label'].apply({-1:0,1:1}.get)
#print(data.head())

data_test = pd.read_csv('testFeat.tsv', sep='\t')#nrows = 20)
data_test['Label'] = data_test['Label'].apply({-1:0,1:1}.get)
...
data_train_u = pd.read_csv('unlabeledFeat.tsv', sep='\t')#nrows = 100)
...
def cotraining(p=50,n=50,u=5000,k=500):
    ...
    while counter < k:
        ...
        ypred1 = acc1[counter+1]*y_pred1[:,0] + acc2[counter+1] *y_pred2[:,0]
        ypred2 = acc1[counter]*y_pred1[:,1] + acc2[counter] *y_pred2[:,1]
        
        classlabel = [1 if y2>=y1 else 0 for y1, y2 in zip(ypred1,ypred2)]
        acc = accuracy_score(y_test,classlabel)
        #print("Counter "+str(counter)+": "+str(acc))
